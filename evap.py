...
rho_air_in = 1/cp.HAPropsSI('Vha', 'T', inletAirTemp, 'P', atmPressure, 'RH', inletAirRH)
...
areaMinCrossSec = finnedLength * (finHeight - nHoles*finCollarOD) * (1 - finThk/finPitch)
areaOuter =  2 * (finHeight*finDepth - (math.pi * finCollarOD**2*nHoles*nRows)/4)*(finnedLength/finPitch) + (nRows*nHoles*finnedLength)/finPitch * (math.pi*(finCollarOD)) * (finPitch-finThk) # total outside area = (fin rectangle area - fin holes area)*(number of fins) + area of pipe outer(with fin) * number of pipes
...
# Colburn j-factor
P3 = -0.361 - (0.042*nRows)/math.log(Re_air_Dc) + 0.158*math.log(nRows*(finPitch/finCollarOD)**(0.41))
...
j = 0.086 * Re_air_Dc**P3 * nRows**P4 * (finPitch/finCollarOD)**P5 * (finPitch/Dh_air)**P6 * (finPitch/transversePitch)**(-0.93)

# Friction factor
F1 = -0.764 + 0.739*(transversePitch/longitudinalPitch) + 0.177*(finPitch/finCollarOD) -0.00758/nRows
F2 = -15.689 + 64.021/(math.log(Re_air_Dc))
F3 = 1.696 - 15.695/math.log(Re_air_Dc)
f_air = 0.0267 * Re_air_Dc**F1 * (transversePitch/longitudinalPitch)**F2 * (finPitch/finCollarOD)**F3
...
# -- refrigerant side ---
nIter = props["Number of Iterations"]
...
for o in range(5):
    ...
    for u in range(nIter-1):
        ...
        latentHeatRef = H_ref_satV - H_ref_satL # latent heat of vapourisatoin of refrigeration
        ...
        if 0 < x[u] < 1:
            ...
            h_satL = 0.023*(Re_satL**0.8)*(Pr_satL**0.4)*(k_satL/tubeID) # Dittus-Boelter
            Co = ((1-x[u])/x[u])**0.8 * (rho_ref_satV/rho_ref_satL)**0.5 # convection number

            def err(T_wall_guess):
                Qguess = h_air * (areaOuter/(nCircuits*nIter)) * (localAirTemp[u] - T_wall_guess)
                qGuess = Qguess/(math.pi * tubeID * dl)
                Bo = qGuess/(G_ref*latentHeatRef) # boiling number
                h_n = h_satL * (0.6683 * Co**(-0.2) * (25 * Fr_lo)**0.3 + 1058 * Bo**0.7 * F_fl)
                h_c = h_satL * (1.136 * Co**(-0.9) * (25*Fr_lo)**0.3 + 667.2 * Bo**0.7 * F_fl)
                href = max(h_c, h_n)
                Qref = href * (math.pi*tubeID*dl) * (T_wall_guess - T[u])
                return Qguess - Qref

            sol = root_scalar(err, bracket=[T[u], localAirTemp[u]], method='brentq') # finding the root to the residual equation using Brent's method
            Twall = sol.root
            ...
            h_nucleate = h_satL * (0.6683 * Co**(-0.2) * (25 * Fr_lo)**0.3 + 1058 * Bo**0.7 * F_fl)
            h_convective = h_satL * (1.136 * Co**(-0.9) * (25*Fr_lo)**0.3 + 667.2 * Bo**0.7 * F_fl)
            ...
            def friedelMultiplier(x):
                ...
                surfTension = cp.PropsSI('surface_tension', 'T', T[u], 'Q', x, refrigerant)
                WeberNumber = (G_ref**2 * tubeID)/(surfTension*rho_ref_satL)
                E = (1-x)**2 + x**2 * (rho_ref_satL*f_satV)/(rho_ref_satV*f_satL)
                Fterm = x**0.78 * (1-x)**0.224
                Hterm = (rho_ref_satL/rho_ref_satV)**0.91 * (mu_ref_satV/mu_satL)**0.19 * (1 - (mu_ref_satV/mu_satL))**0.7
                return E + (3.24*Fterm*Hterm)/(Fr_lo**0.045 * WeberNumber**0.035)
            ...
            if Re_v_sh < 3000:
                ...
                Nu_v_sh = ((f_ref_v/8)*(Re_v_sh-1000)*Pr_v_sh)/(1 + 12.7 * (f_ref_v/8)**(1/2) * (Pr_v_sh**(2/3) - 1))
            ...
            if T[u] >= localAirTemp[u] - 0.001:  # 0.001 tolerance for float precision
                ...
                try:
                    ...
                    sol = root_scalar(err, bracket=[T[u], localAirTemp[u]], method='brentq')
                    Twall = sol.root
                    Qref = h_ref * (math.pi*tubeID*dl) * (Twall - T[u])
                except ValueError:
...
plt.plot(h_f_sat*1e-3, np.log(Psat / 1e5), 'b-', label='Saturated Liquid')
plt.plot(h_g_sat*1e-3, np.log(Psat / 1e5), 'r-', label='Saturated Vapor')
plt.plot(H*1e-3,np.log(P*1e-5))
plt.show()
